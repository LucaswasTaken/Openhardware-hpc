...
int main(int argc, char** argv){
    ...
    double * distances_global = NULL;
    int * recvcounts = NULL;
    int * displs = NULL;
    if(rank==0){
        distances_global = new double[npoints];
    ...
    MPI_Gatherv(distances, num_elems_proc, MPI_DOUBLE, distances_global, recvcounts, displs, MPI_DOUBLE,0, MPI_COMM_WORLD);
